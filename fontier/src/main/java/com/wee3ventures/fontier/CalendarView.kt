...
import com.wee3ventures.fontier.utils.Console
...
class CalendarView : LinearLayout {
    ...
    fun markCircleImage1(date: Date,color: Any? = "#EB5757") {
        val calendar = Calendar.getInstance()
        calendar.time = date
        Console.log("LIBRARY",message = "MODEL_1_DATE:\t$date, Calendar:\t${calendar.time}")
        ...
        if (lastSelectedDayCalendar != null && areInTheSameDay(
                calendar,
                lastSelectedDayCalendar!!
            )
        ) {
            DrawableCompat.setTint(circleImage1.drawable, ContextCompat.getColor(context, R.color.roboto_calendar_selected_day_font))
        } else {
            DrawableCompat.setTint(circleImage1.drawable, Color.parseColor("$color"))
        }
    }

    fun markCircleImage2(date: Date, color : Any ?= "#55c6da") {
        val calendar = Calendar.getInstance()
        calendar.time = date
        Console.log("LIBRARY",message = "MODEL_2_DATE:\t$date, Calendar:\t${calendar.time}")
        ...
        if (lastSelectedDayCalendar != null && areInTheSameDay(calendar, lastSelectedDayCalendar!!)) {
            DrawableCompat.setTint(circleImage2.drawable, ContextCompat.getColor(context, R.color.roboto_calendar_selected_day_font))
        } else {
            DrawableCompat.setTint(circleImage2.drawable, Color.parseColor("$color"))
        }
    }
    ...
    fun showDateTitle(show: Boolean) {
        if (show) {
            robotoCalendarMonthLayout!!.visibility = View.VISIBLE
        } else {
            robotoCalendarMonthLayout!!.visibility = View.GONE
        }
    }
